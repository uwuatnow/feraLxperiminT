...
{
    play((double)x, (double)y, 0.0, noSpam, volume);
}

void SoundFxEngine::Fx::play(double x, double y, bool noSpam, float volume)
{
    play(x, y, 0.0, noSpam, volume);
}

void SoundFxEngine::Fx::play(double x, double y, double z, bool noSpam, float volume)
{
    // Calculate distance to listener in double precision
    // Coordinate mapping: game X → audio X, game Y → audio Z (depth), game Z → audio Y (height)
    double dx = x - host->listenerX;
    double dy = y - host->listenerZ; // game Y (depth) → audio Z
    double dz = z - host->listenerY; // game Z (height) → audio Y
    ...
    for (auto s : instances) {
        if (!ma_sound_is_playing(s)) {
            ...
            // Set 3D position: x → X, y → Z (depth), z → Y (height)
            ma_sound_set_position(s, (float)x, (float)z, (float)y);
